...
//
// Stages: Checkout → Build → Archive → Approve PROD (prod only) → Deploy
//
...
//   • ไม่มี stage 'Unit test' (เหมือน dninhouse) — เคยมีแล้วถอดออก ไม่ใช่ยังไม่ได้ทำ
//     script `test:headless` + karma.conf.js (ChromeHeadlessNoSandbox) + puppeteer ยังอยู่ในรีโป
//     รันเองได้ที่เครื่อง; ถ้าจะเอากลับเข้า pipeline ให้ใส่คั่นระหว่าง Build กับ Archive
//     (artifact จะได้เป็นของที่เทสต์ผ่าน) แล้วส่ง -e NODE_OPTIONS=${NODE_OPTS} ให้ container ด้วย —
//     Karma bundle ทั้งแอปเหมือนกัน เสี่ยง heap พอกัน
