...
import { TAccrualBranchReportRow } from '../../../shared/types/other-income.type';
...
function flatten(rows: TAccrualBranchReportRow[]): TBranchReportFlatRow[] {
...
const exportConfig: TAoaConfig<TBranchReportFlatRow> = {
  sheetName: 'Light Box',
  config: [
...
export class AccrualBranchReportPageComponent {
  ...
  rows = signal<TAccrualBranchReportRow[]>([])
  ...
  constructor() {
    this.route.queryParamMap.pipe(
      switchMap(params => {
        ...
        return this.api.getAccrualBranchReport({
  ...
  exportExcel(): void {
    const mapper = this.xlsx.convertJsonToWorkbook<TBranchReportFlatRow>(exportConfig)
    const exporter = this.xlsx.exportWorkbook(`Light Box ${new Date().toISOString().split('T')[0]}`)
