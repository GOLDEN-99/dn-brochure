...
import { FreeItemStateWorklistPageComponent } from './free-item-state-worklist-page.component';
...
describe('FreeItemStateWorklistPageComponent', () => {
  let component: FreeItemStateWorklistPageComponent;
  let fixture: ComponentFixture<FreeItemStateWorklistPageComponent>;
  let api: jasmine.SpyObj<OtherIncomeAccountApiService>;
  let router: jasmine.SpyObj<Router>;
  let xlsx: jasmine.SpyObj<XLSXReportService>;
  let queryParamMap$: BehaviorSubject<ReturnType<typeof convertToParamMap>>;

  const row = (over: Partial<TFreeItemStateRow> = {}): TFreeItemStateRow => ({
    freeItemId: 1,
    settlementId: 172,
    contractId: 87,
    contractType: 'PROMO',
    ...
    incomeLabelName: 'จ่ายเป็นสินค้า',
    orderNumb: '058047',
  ...
  beforeEach(async () => {
    ...
    api.getFreeItemStates.and.returnValue(of([row()]));
    api.checkFreeItem.and.returnValue(of(undefined));
    api.deleteFreeItemState.and.returnValue(of(undefined));
    ...
    await TestBed.configureTestingModule({
      imports: [FreeItemStateWorklistPageComponent],
      providers: [
    ...
    fixture = TestBed.createComponent(FreeItemStateWorklistPageComponent);
  ...
  it('loads rows on init with the default DN comp type', () => {
    ...
    expect(api.getFreeItemStates).toHaveBeenCalledWith({
  ...
  describe('PO / RC columns', () => {
    ...
    it('renders a dash when either date is null', () => {
      api.getFreeItemStates.and.returnValue(of([row({ orderDate: null, receDate: null })]));
      component.refresh();
      fixture.detectChanges();
  ...
  describe('check', () => {
    it('marks a row checked and reloads the list', () => {
      api.getFreeItemStates.calls.reset();

      component.check(row());

      expect(api.checkFreeItem).toHaveBeenCalledWith(172, 1, 'account_user');
      expect(api.getFreeItemStates).toHaveBeenCalled();
      expect(component.checkingId()).toBeNull();
    });

    it('surfaces an error and clears the pending id when the check fails', () => {
      api.checkFreeItem.and.returnValue(throwError(() => new Error('boom')));
  ...
  describe('remove', () => {
    it('deletes a row and reloads the list', () => {
      ...
      expect(api.deleteFreeItemState).toHaveBeenCalledWith(172, 1);
      expect(api.getFreeItemStates).toHaveBeenCalled();
    });

    it('surfaces an error when the delete fails', () => {
      api.deleteFreeItemState.and.returnValue(throwError(() => new Error('boom')));
  ...
  describe('error handling', () => {
    it('shows an error message and keeps loading false when the fetch fails', () => {
      api.getFreeItemStates.and.returnValue(throwError(() => new Error('boom')));
      component.refresh();
